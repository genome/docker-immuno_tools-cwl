...
outdir=$dir/$name.optitype;
mkdir $outdir

# convert cram to bam:
samtools1.2 view -b $cram > $TEMPDIR/$name.unsorted.bam
...
bedtools bamtofastq -fq $TEMPDIR/$name.q.fwd.fastq -fq2 $TEMPDIR/$name.q.rev.fastq -i $TEMPDIR/$name.qsorted.bam 2>/dev/null;
...
#/usr/bin/bwa0.7.12 index /gscmnt/gc6134/cancer-genomics/medseq/shared/Software/Optitype/Data/hla_reference_dna.fasta;

echo step 1
#1 Align forward reads to reference HLA locus sequence
/usr/bin/bwa0.7.12 mem -t 4 $dnaref $TEMPDIR/$name.q.fwd.fastq > $TEMPDIR/$name.aln.fwd.sam # use bwa mem, store output IN TEMP, and skip samse step
rm -f $TEMPDIR/$name.q.fwd.fastq;

echo step 2
#2 Align reverse reads to reference HLA locus sequence
/usr/bin/bwa0.7.12 mem -t 4 $dnaref $TEMPDIR/$name.q.rev.fastq > $TEMPDIR/$name.aln.rev.sam # use bwa mem, store output IN TEMP, and skip samse step
rm -f $TEMPDIR/$name.q.rev.fastq

echo step 3
# select only the mapped reads from the sam files:
samtools view -S -F 4 $TEMPDIR/$name.aln.fwd.sam > $TEMPDIR/$name.aln.map.fwd.sam
samtools view -S -F 4 $TEMPDIR/$name.aln.rev.sam > $TEMPDIR/$name.aln.map.rev.sam
echo output step 3:
